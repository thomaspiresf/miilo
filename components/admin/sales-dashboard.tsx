...
function RecentOrders({ orders }: { orders: DashOrder[] }) {
  ...
  if (orders.length === 0)
    return <p className="text-sm text-muted">Nenhum pedido no período.</p>;
            ...
            {isOpen && (
              <div className="border-t border-border/60 bg-black/[0.015] px-4 py-3 text-sm">
                <div className="mb-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted">
                  ...
                  <span className="sm:hidden">{o.email}</span>
                </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        ...
        <RecentOrders orders={view.recent} />
      </section>
    </div>
  );
}
